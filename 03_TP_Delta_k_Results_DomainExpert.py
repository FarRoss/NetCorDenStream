...
for baseNode in baseNodes:
    ...
    for dataset in config['dataset']['availableDataset']:
        ...
        for i in d_lst:
            prfd = {}
            if len(results[0][i][1]['Precision']) and len(results[0][i][2]['Precision']) and len(
                    results[0][i][3]['Precision']) \
                    and len(results[0][i][4]['Precision']) and len(results[0][i][5]['Precision']) > 1:
            ...
            if len(results[0][i][1]['Recall']) and len(results[0][i][2]['Recall']) and len(results[0][i][3]['Recall']) \
                    and len(results[0][i][4]['Recall']) and len(results[0][i][5]['Recall']) > 1:
                prfd["Recall"] = [max(results[0][i][1]['Recall']),
                                  max(results[0][i][2]['Recall']),
                                  max(results[0][i][3]['Recall']),
                                  max(results[0][i][4]['Recall']),
                                  max(results[0][i][5]['Recall'])]
            if len(results[0][i][1]['False']) and len(results[0][i][2]['False']) and len(results[0][i][3]['False']) \
                    and len(results[0][i][4]['False']) and len(results[0][i][5]['False']) > 1:
            ...
            if len(results[0][i][1]['Delay']) and len(results[0][i][2]['Delay']) and len(results[0][i][3]['Delay']) \
                    and len(results[0][i][4]['Delay']) and len(results[0][i][5]['Delay']) > 1:
        ...
        with open(path + 'baseNode_' + baseNode + '/' + features + '/' + 'Delta_k' '/' + 'TP_' + dataset + '.json',
                  'w') as outfile:
