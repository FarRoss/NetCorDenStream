...
for baseNode in baseNodes:
    ...
    for dataset in config['dataset']['availableDataset']:
        ...
        timProx = timeproximity(baseNode, truth, delta)
        results = timProx.getStats(topo, kNeighbors_lst, delta_lst, dataset)

        delta_prf = {}
        for i in range(1, 6):
            prfd = {}
            if len(results[0][i][5]['Precision']) and len(results[0][i][15]['Precision']) and len(
                    results[0][i][30]['Precision']) \
                    and len(results[0][i][40]['Precision']) and len(results[0][i][55]['Precision']) > 1:
                prfd["Precision"] = [max(results[0][i][5]['Precision']),
                                     max(results[0][i][15]['Precision']),
                                     max(results[0][i][30]['Precision']),
                                     max(results[0][i][40]['Precision']),
                                     max(results[0][i][55]['Precision'])]
            if len(results[0][i][5]['Recall']) and len(results[0][i][15]['Recall']) and len(results[0][i][30]['Recall']) \
                    and len(results[0][i][40]['Recall']) and len(results[0][i][55]['Recall']) > 1:
                prfd["Recall"] = [max(results[0][i][5]['Recall']),
                                  max(results[0][i][15]['Recall']),
                                  max(results[0][i][30]['Recall']),
                                  max(results[0][i][40]['Recall']),
                                  max(results[0][i][55]['Recall'])]
            if len(results[0][i][5]['False']) and len(results[0][i][15]['False']) and len(results[0][i][30]['False']) \
                    and len(results[0][i][40]['False']) and len(results[0][i][55]['False']) > 1:
            ...
            if len(results[0][i][5]['Delay']) and len(results[0][i][15]['Delay']) and len(results[0][i][30]['Delay']) \
                    and len(results[0][i][40]['Delay']) and len(results[0][i][55]['Delay']) > 1:
        ...
        with open(path + 'baseNode_' + baseNode + '/' + features + '/' + 'k_Delta' '/' + 'TP_' + dataset + '.json',
                  'w') as outfile:
